...
namespace VCX::Labs::Animation {
    ...
    struct IKStatistics {
        int total_iters_ccd    = 0;
        int total_iters_fabrik = 0;
        int count_ccd          = 0;
        int count_fabrik       = 0;

        void Reset() { *this = IKStatistics(); }

        double AverageCCD() const { return double(total_iters_ccd) / count_ccd; }
        double AverageFABRIK() const { return double(total_iters_fabrik) / count_fabrik; }
    };

    inline IKStatistics g_IKStats;
    ...
    Common::CaseRenderResult CaseInverseKinematics::OnRender(std::pair<std::uint32_t, std::uint32_t> const desiredSize) {
        ...
        if (! _stopped) {
            ...
            if (ik_type == IKType::CCD_IK) {
                InverseKinematicsCCD(ik_system, target, 10000, 1e-4f);
