...
def main():
    ...
    # Build command line arguments,这里包括系统名称，问题名称，积分器类型，子空间模型路径等
    ...
    # Arguments specific to this program，可选积分器和子空间模型
    ...
    ## Integrator setup
    int_opts = {}
    int_state = {} # 后文可以看到，这里存储了q_t(潜在空间位置)，q_tm1（上一步位置），qdot_t（速度）
    integrators.initialize_integrator(int_opts, int_state, args.integrator)

    ## State of the system

    # UI state
    run_sim = False             # 是否运行模拟
    eval_energy_every = True    # 是否每次评估能量（系统势能）
    update_viz_every = True     # 是否每次更新可视化
    ...
    def reset_state():
        if use_subspace:
            int_state['q_t'] = base_latent              # 这里的q_t是初始预设潜在空间中的位置
        else:
            int_state['q_t'] = system_def['init_pos']   # 这里的q_t是初始系统位置
    ...
    if use_subspace:
        ...
        subspace_fn = None
    # 这里传递给积分器的subspace_fn函数，用于将潜在空间状态映射到系统空间，也就是训练的关键点
    ps.set_automatically_compute_scene_extents(False) # 这里先关闭自动计算范围
    reset_state()  # also creates initial viz   # 重置状态，并创建初始可视化
